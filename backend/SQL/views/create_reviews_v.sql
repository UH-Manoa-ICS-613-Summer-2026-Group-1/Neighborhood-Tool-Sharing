...
SELECT
    ...
    reviews.reservation_id AS reservation_id,
    reviews.reviewee_id AS reviewee_id,
...
LEFT JOIN photos reviewer_photo ON reviewer.photo_id = reviewer_photo.id
JOIN users reviewee ON reviews.reviewee_id = reviewee.id
LEFT JOIN photos reviewee_photo ON reviewee.photo_id = reviewee_photo.id;
...
COMMENT ON COLUMN reviews_v.reviewee_photo_url IS 'The address hosting the actual image file of the user being reviewed';
...
COMMENT ON COLUMN reviews_v.reviewer_photo_url IS 'The address hosting the actual image file of the user who wrote the review';
